 ...
 *
这是个简单代码示例，该程序在SS  TWR距离测量交换中作为发起者角色，
该程序发送一个"poll"帧(记录poll的TX时间戳),
之后等待一个来自"DS TWR responder"的响应消息
那部分示例代码与本程序一起完成消息交换，
响应消息包含远方响应者的poll的时间戳（The response message contains the remote responder's time-stamps of poll）
RX, 和响应TX.
使用这些数据和本地的时间戳,
 (of poll TX and response RX),
 该示例程序计算出飞行时间的值（the time-of-flight over-the-air）,
 然后,两个设备间的估计距离写到LCD.
 *
 * @attention
 *
...
static dwt_config_t config = {
    ...
    DWT_PAC8,        /* yuanqd:报头捕获数据块，
是在DW1000模块数据接收过程中,
需要检测接收数据的一种机制。
报头捕获数据块，是指在接收器里面，
在报头检测过程中收集在一起的一组报头符号，
可以通过改变DRX_TUNE2来设置该数据块的大小。
（Preamble acquisition chunk size. Used in RX only. */
    9,               /* yuanqd:TX的前导码，只TX使用
	// preamble code. Used in TX only. */
    9,               /* yuanqd:RX的前导码，只RX使用
	// preamble code. Used in RX only. */
    0,               /* yuanqd:使用非标准的帧定界符
	(Start Frame Delimiter)(布尔值)
	//Use non-standard SFD (Boolean) */
    DWT_BR_6M8,      /* 数据速率//Data rate. */
    DWT_PHRMODE_STD, /* yuanqd:物理层报头模式//PHY header mode. */
    (129 + 8 - 8)    /* yuanqd:帧定界符超时
	(preamble length + 1 + SFD length - PAC size)
	//SFD timeout (preamble length + 1 + SFD length - PAC size). 
	Used in RX only. */
};

/* yuanqd:64MHz的PRF的默认天线时延时，
见下面的NOTE2。
PRF（pulse repetition frequency）即脉冲重复频率。
...
/* yuanqd:用于存放接收到的响应信息的缓存
//Buffer to store received response message.
 * 这个大小根据本示例代码准备处理的最长帧调整
 //Its size is adjusted to longest frame 
 that this example code is supposed to handle. */
#define RX_BUF_LEN 20
static uint8 rx_buffer[RX_BUF_LEN];

/* yuanqd:保持寄存器状态的copy用以引用，
这样读取器就可以在断点时检查
//Hold copy of status register state here for reference, 
so reader can examine it at a breakpoint. */
static uint32 status_reg = 0;

/* yuanqd:UWB微秒（uus）转设备时间单位的转换因子（参数）
//UWB microsecond (uus) to device time unit (dtu, around 15.65 ps) conversion factor.
 * 1 uus = 512 / 499.2 µs  and 1 µs = 499.2 * 128 dtu. */
#define UUS_TO_DWT_TIME 65536

/* yuanqd:帧间延迟，单位为UWB的微秒，
见后文NOTE 1
//Delay between frames, in UWB microseconds. See NOTE 1 below. */
#define POLL_TX_TO_RESP_RX_DLY_UUS 140
/* yuanqd:接收响应过期时间，见后面NOTE 5
...
/* yuanqd: 在LCD上显示测量距离的字符串（最大16个字符）
//String used to display measured distance on LCD screen 
...
int main(void)
{
    ...
    /* yuanqd:设置期望的响应时延和超时时长。
	见NOTE 1和 NOTE 5
	//Set expected response's delay and timeout. 
	See NOTE 1 and 5 below.
     * yuanqd:本例中只是使用同样的时延和超时时常来处理一个进来的帧，
	 这些值可以在这里一次性设置
    ...
    while (1)
    {
        ...
        /* yuanqd:开始发送，意味着期待一个响应，
		所以在帧发送后，
		并等待dwt_setrxaftertxdelay（）函数设置的延时时间过去后 ，
		接收机制会被自动使能
		//Start transmission, 
		indicating that a response is expected so that reception is enabled automatically after the frame is sent and the delay
         * set by dwt_setrxaftertxdelay() has elapsed. */
        dwt_starttx(DWT_START_TX_IMMEDIATE | DWT_RESPONSE_EXPECTED);

        /* yuanqd:我们假设发送的消息正确到达,
		一个帧的接收或者错误超时的poll，
		见NOTE 8
        ...
        /* yuanqd:完成一个poll消息的传输后增加帧序列号（modulo 256）
        ...
        if (status_reg & SYS_STATUS_RXFCG)
        {
            ...
            /* yuanqd:检查这个帧是从伙伴"SS TWR responder"
			发来的期望的回应
        ...
        {
            /* yuanqd:清除DW1000状态寄存器上的RX错误事件
			//Clear RX error events in the DW1000 status register. */
            dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_ALL_RX_ERR);
        }

        /* yuanqd:在两次距离交换之间执行一个延时
 ...
 * @fn resp_msg_get_ts()
 *
 * @brief yuanqd:从响应消息里读取一个给定的时间戳的值。 
 在响应消息的时间戳字段里，
 越不重要的字节地址越低
 //Read a given timestamp value from the response message. 
 In the timestamp fields of the response message, 
 the least significant byte is at the lower address.
 *
 * @param  ts_field  yuanqd:时间戳字段的第一个字节指针，
 用来获得时间戳值//pointer on the first byte of the timestamp field to get
 *         ts  timestamp value
 *
 * @return none
 */
